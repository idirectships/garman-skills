---
name: hook-developer
description: "Reference and creation guide for Claude Code hooks — schemas, registration, blocking/injection patterns, TypeScript templates, testing. Use when: 'write a hook', 'create a hook', 'how do I block a tool call', 'inject context into a prompt', 'hook input/output format'."
---
...
5. **Test manually** - `echo '{}' | /absolute/path/to/hook.sh` before relying on it
6. **Check outputs first** - `ls .claude/` before editing code
7. **Detached spawn hides errors** - add logging to debug

## See Also

- `/debug-hooks` - Systematic debugging workflow
See Also: /debug-hooks skill — systematic debugging workflow
